...
debug(__dirname[__dirname.length - 1]);
...
class App {
    ...
    /*
      configureRoutes() {
          this.app.use('/:version', (req, res) => {
              const ver = req.params.version; // Obtén la versión de la URL
              this.version = ver; // Obtén la versión de la URL
              //    next();
          });
  ...
  */
    configureRoutes() {
        ...
        this.app.use('/api/:version', (req, res, next) => {
            ...
            if (regex.test(ver)) {
                this.version = ver;
                res.status(200).send(`${ver} esla version actual`);
                //next();
            } else {
                res.status(400).send('...');
            }
        ...
        //        app.use(express.static((req, res) => req.pathToVersion));
